...
int decode_and_show(void *arg)
{
     ...
     // ...
      ...
      {
           LOGE( "receive video frame error\n");
           return -1;
      }
     ...
     LOGI("video frame pts = %lf\n", pts);
     LOGI( "显示里面：ps->delay = %d\n", ps->delay);
...
double get_delay(PlayerState *ps)
{
    ...
    LOGI("cur_audio_clock：%d",cur_audio_clock);
