...
double get_rotation(AVStream *st)
{
    ...
    //int s = AVCODEC_MAX_AUDIO_FRAME_SIZE;
...
int main(int argc, char** argv)
{
    ...
    double r = get_rotation(pFormatCtx->streams[streamIdx]);
    LOGI("视频的旋转角度：%f",r);
    ...
    //设置window属性  SDL_WINDOW_RESIZABLE|SDL_WINDOW_FULLSCREEN|
    ...
    LOGE("视频的宽和高:%d %d",pCodecCtx->width,pCodecCtx->height);
    //默认全屏可以不用设置
    sdlRect.x = 0;
    sdlRect.y = 0;
    sdlRect.w = SCREEN_W;
        ...
        if (packet->stream_index == streamIdx) {
            ...
            if(getPacketCode == 0) {
                ...
                if (getFrameCode == 0) {
                    ...
                    SDL_UpdateYUVTexture(sdlTexture, &sdlRect, pFrame_out->data[0], pFrame_out->linesize[0],
                    ...
                    SDL_RenderCopy(renderer, sdlTexture, NULL, &sdlRect); //这个函数&sdlRect 参数可以设置播放屏幕的大小
...
int test(){
    ...
    SDL_CreateThread_REAL(decode_video1,"decode_video1",NULL);
}