...
void audio_callback(void *userdata, Uint8 *stream, int len) {
    ...
    LOGI("len:%d",len);
    while (len > 0) {
        LOGI("audio_buf_index:%d,audio_buf_size:%d",is->audio_buf_index,is->audio_buf_size);
        if (is->audio_buf_index >= is->audio_buf_size) {
            audio_data_size = audio_decode_frame(is);
            LOGI("audio_data_size:%d",audio_data_size);
...
int stream_component_open(VideoState *is, int stream_index) {
	...
	while(SDL_OpenAudio(&wanted_spec, &spec) < 0) {
		LOGE(stderr, "SDL_OpenAudio (%d channels): %s\n", wanted_spec.channels, SDL_GetError());
		wanted_spec.channels = next_nb_channels[FFMIN(7, wanted_spec.channels)];
        LOGI("channels:%d",wanted_spec.channels);
		if(!wanted_spec.channels) {
			LOGE(stderr, "No more channel combinations to tyu, audio open failed\n");
			return -1;
		}
	...
	if (spec.format != AUDIO_S16SYS) {
        LOGE(stderr, "SDL advised audio format %d is not supported!\n", spec.format);
		return -1;
	}
	if (spec.channels != wanted_spec.channels) {
		wanted_channel_layout = av_get_default_channel_layout(spec.channels);
		if (!wanted_channel_layout) {
            LOGE(stderr, "SDL advised channel count %d is not supported!\n", spec.channels);
			return -1;
		}
	}

//    LOGE(stderr, "%d: wanted_spec.format = %d\n", __LINE__, wanted_spec.format);
//    LOGE(stderr, "%d: wanted_spec.samples = %d\n", __LINE__, wanted_spec.samples);
//    LOGE(stderr, "%d: wanted_spec.channels = %d\n", __LINE__, wanted_spec.channels);
//    LOGE(stderr, "%d: wanted_spec.freq = %d\n", __LINE__, wanted_spec.freq);
//
//    LOGE(stderr, "%d: spec.format = %d\n", __LINE__, spec.format);
//    LOGE(stderr, "%d: spec.samples = %d\n", __LINE__, spec.samples);
//    LOGE(stderr, "%d: spec.channels = %d\n", __LINE__, spec.channels);
//    LOGE(stderr, "%d: spec.freq = %d\n", __LINE__, spec.freq);
...
static int decode_thread(void *arg) {
    ...
    if (is->audioStream < 0) {
        LOGE(stderr, "%s: could not open codecs\n", is->filename);
        goto fail;
    }
    // main decode loop
    for(;;) {
